...
struct StudySessionViewModelTests {
    ...
    @Test("Load cards creates study session")
    func loadCardsCreatesStudySession() async throws {
        ...
        #expect(sessions[0].cardsReviewed == 0)
        #expect(sessions[0].mode == .scheduled)
    }
    ...
    func loadCardsEmptyDeckNoSession() async throws {
        ...
        // Verify no StudySession was created
        let descriptor = FetchDescriptor<StudySession>()
        let sessions = try context.fetch(descriptor)
    ...
    @Test("Complete session finalizes study session")
    func completeSessionFinalizesStudySession() async throws {
        ...
        // Submit rating to complete session
        guard let card = viewModel.currentCard else {
            #expect(Bool(false), "Expected currentCard to be non-nil")
            return
        }
        ...
        #expect(sessions[0].cardsReviewed == 1)
        #expect(!sessions[0].isActive)
    }

    @Test("Reviews are linked to study session")
    func reviewsLinkedToStudySession() async throws {
        ...
        let card2 = createTestFlashcard(context: context, word: "card2", stateEnum: FlashcardState.learning.rawValue, dueOffset: -3600)
        try context.save()
        ...
        // Submit ratings
        await viewModel.submitRating(2, card: card1)
        await viewModel.submitRating(2, card: card2)

        // Verify reviews are linked to session
        let reviewDescriptor = FetchDescriptor<FlashcardReview>()
        let reviews = try context.fetch(reviewDescriptor)

        #expect(reviews.count == 2)
        #expect(reviews.allSatisfy { $0.studySession?.id == session.id })
    }

    @Test("Reset session finalizes previous session")
    func resetSessionFinalizesPrevious() async throws {
        ...
        // Verify previous session was finalized
        let descriptor = FetchDescriptor<StudySession>()
        let sessions = try context.fetch(descriptor)
    ...
    @Test("Cleanup finalizes active session")
    func cleanupFinalizesActiveSession() async throws {
        ...
        // Call cleanup (simulating view dismissal)
        viewModel.cleanup()
    ...
    @Test("Cram mode creates cram session")
    func cramModeCreatesCramSession() async throws {
        ...
        // Verify StudySession was created with cram mode
        let descriptor = FetchDescriptor<StudySession>()
        let sessions = try context.fetch(descriptor)

        #expect(sessions.count == 1)
        #expect(sessions[0].mode == .cram)
    }

    @Test("Learning mode creates learning session")
    func learningModeCreatesLearningSession() async throws {
        ...
        // Verify StudySession was created with learning mode
        let descriptor = FetchDescriptor<StudySession>()
        let sessions = try context.fetch(descriptor)

        #expect(sessions.count == 1)
        #expect(sessions[0].mode == .learning)
    }

    @Test("Session creation failure does not block study")
    func sessionCreationFailureDoesNotBlock() async throws {
        let context = freshContext()
        try context.clearAll()
