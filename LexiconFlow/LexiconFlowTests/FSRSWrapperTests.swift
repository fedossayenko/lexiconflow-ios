...
struct FSRSWrapperTests {
    ...
    func dtoNewCardValues() async throws {
        ...
        let flashcard = Flashcard(word: "test", definition: "a test", phonetic: "tɛst")
        try context.save()
        let now = Date()
    ...
    func dtoScheduledDifferences() async throws {
        ...
        for rating in 0...3 {
            try context.clearAll()
            let flashcard = Flashcard(word: "card\(rating, definition: "a test", phonetic: "tɛst")")
            try context.save()
            let result = try await FSRSWrapper.shared.processReview(
                flashcard: flashcard,
                rating: rating,
    ...
    func dtoStabilityIncrease() async throws {
        let context = TestContext.clean()
        try context.clearAll()
        let flashcard = TestFixtures.createFlashcard(context: context, state: .review, stability: 10.0, save: false)
        try context.save()
    ...
    func dtoDifficultyAdjustment() async throws {
        ...
        try context.clearAll()
        let flashcard1 = Flashcard(word: "again_test", withState: true, definition: "a test", phonetic: "tɛst")
        try context.save()
        ...
        try context.clearAll()
        let flashcard2 = Flashcard(word: "easy_test", withState: true, definition: "a test", phonetic: "tɛst")
        try context.save()
    ...
    func dtoStateTransitions() async throws {
        ...
        try context.clearAll()
        let flashcard1 = Flashcard(word: "new_to_learning", withState: false, definition: "a test", phonetic: "tɛst")
        try context.save()
        ...
        try context.clearAll()
        let flashcard2 = Flashcard(word: "review_to_relearning", withState: true, definition: "a test", phonetic: "tɛst")
    ...
    func stateEnumValidValues() async throws {
        ...
        for stateEnum in [FlashcardState.new.rawValue, FlashcardState.learning.rawValue,
            ...
            let flashcard = Flashcard(word: "state_\(stateEnum, definition: "a test", phonetic: "tɛst")", withState: true)
